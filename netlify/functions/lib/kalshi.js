...
//
// Kalshi market fields used: yes_bid/yes_ask/no_bid/no_ask (integer cents 0-100),
...
// Cents → dollars; an ask of 0/absent means "no offers" on Kalshi, so treat it as null
// and let the caller derive NO from the YES bid.
const askDollars = (v) => (Number.isFinite(v) && v > 0 && v < 100 ? v / 100 : null);
const bidDollars = (v) => (Number.isFinite(v) && v >= 0 && v < 100 ? v / 100 : null);
...
export function parseMarketToBook(m) {
  ...
  return {
    ...
    loInt, hiInt,
    no_ask: askDollars(m.no_ask), yes_bid: bidDollars(m.yes_bid),
    yes_ask: askDollars(m.yes_ask), no_bid: bidDollars(m.no_bid),
  };
}
